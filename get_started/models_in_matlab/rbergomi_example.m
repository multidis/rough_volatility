...
%% Define a rough Bergomi model (and inspect the object)
...
% Define a model:
model = rBergomiClass('H',0.1,'xi',0.15^2,'eta',2.3,'rho',-0.8,'s0',100);
...
% Compute price and standard errors:
k = (-0.3:0.01:0.2)';
T = 0.5;
...
model = rBergomiClass('H',0.1,'xi',xi_curve,'eta',2.3,'rho',-0.8,'s0',100);
model.pricerSettings.N = 10000;
...
% The explanations given here will however be somewhat limited - you should 
% consult the code for the full details.
...
% And if we want to use conditional Monte Carlo:
model.pricerSettings.price_estimation.conditional_monte_carlo = true;
