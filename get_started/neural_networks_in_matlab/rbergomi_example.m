...
% The vector below shows the grid points of the piecewise flat forward variance curve sections:
Txi
...
% Compute true prices:
model_true = rBergomiClass('H',H,'rho',rho,'eta',eta,'xi',xi);
...
rho = -0.90;
xi = 0.15^2;
par_true = [H;eta;rho;repmat(xi,27,1)];
...
uniqT = unique(T_obs);
grpPart = MergeForwardVarianceCurve(Txi,uniqT).';
idxConvParToNN = [1,2,3,grpPart + 3];

% Set the initial guess:
x0 = [0.2,2.5,-0.8,ones(1,max(grpPart))*0.3.^2].';
