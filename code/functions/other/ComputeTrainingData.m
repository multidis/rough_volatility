...
% 
% Remarks: 
%   o Prices are computed for call options only. One will have to
%     post-process the data if e.g. implied volatility is desired instead.
%
%   o Standard errors will also be returned if model.pricerSettings 
%     is a 'MonteCarloPricerSettingsClass'.
%
%   o We assume the model has zero interest rate and dividend. If this is
%     not the case this will automatically overwritten in the input model
%     object.
%
% Parameters:
...
%
    ...
    if use_sobol
        ...
        % Determine if we need to also sample from the 'xi_special'
        % distribution (this distribution will still uses pseudo random
        % numbers regardless of how the parameter use_sobol is set):
        
        idx_xi_special = false(size(params));
        for i=1:size(params,2)
           idx_xi_special(i) = strcmpi(samplingSettings{i}.method,'xi_special');
        end
        
        if sum(idx_xi_special) == 0
            p = sobolset(nParamTotal);
        elseif sum(idx_xi_special) == 1
            nParamXi = size(model.(params{idx_xi_special}).gridpoints,1);
            p = sobolset(nParamTotal - nParamXi);
        else
            error(['GenNewSamples: The sampling method ''xi_special'' can ',...
                  'only be used once and that for a CurveClass type ',...
                  'parameter with the name ''xi''.']);
        end
